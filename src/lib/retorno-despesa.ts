...
export type RetornoDespesaResult =
  | { ok: false; status: number; error: string }
  | { ok: true; amount: number; perPerson: number; removed?: boolean; crew?: number; items?: number };

// Sincroniza a despesa "Material perdido" do navio com o retorno confirmado:
// lost_qty × valor unitário de cada item. Miolo do POST /api/retorno/despesa,
// extraído pra também rodar quando o valor unitário é editado no Pagamento de
// Navios (/api/retorno/valor) — lá o chamador é do Financeiro e nem sempre tem
// a permissão de embarque exigida pela rota.
//
// VALOR UNITÁRIO: o padrão vem do Almoxarifado (stock_items.unit_value), mas o
// navio pode ter valor próprio em ship_material_values (editado no modal do
// Pagamento de Navios) — quando existe, vale o do navio. Editar lá não mexe no
// estoque nem nos outros navios.
//
...
// já esconde o rancho do bloco "Retorno de material".
//
...
// acompanha). Perda zerada na edição remove a despesa.
export async function syncRetornoDespesa(shipId: string, team: string, actor: string): Promise<RetornoDespesaResult> {
  const ship = await prisma.ship.findUnique({ where: { id: shipId } });
  if (!ship) return { ok: false, status: 404, error: "Navio não encontrado" };
  ...
  if (!ret) {
    return { ok: false, status: 404, error: "Nenhum retorno confirmado para este navio/equipe" };
  }
  ...
  const [items, overrides] = ids.length
    ? await Promise.all([
        prisma.stockItem.findMany({ where: { id: { in: ids } }, select: { id: true, team: true, unit_value: true } }),
        prisma.shipMaterialValue.findMany({ where: { ship_id: shipId, stock_item_id: { in: ids } } }),
      ])
    : [[], []];
  const valueById = new Map(items.map((i) => [i.id, i.unit_value || 0]));
  // Valor específico deste navio ganha do valor de estoque.
  for (const ov of overrides) valueById.set(ov.stock_item_id, ov.unit_value);
  ...
  if (total <= 0) {
    ...
    return { ok: true, amount: 0, perPerson: 0, removed: !!existing };
  }
  ...
  return { ok: true, amount: total, perPerson, crew: people.size, items: parts.length };
}
